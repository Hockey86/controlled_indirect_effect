...
if __name__=='__main__':
    ...
    res.loc[res.anx=='not anxious at all', 'anx'] = 0
    res.loc[res.anx=='a little anxious', 'anx'] = 1
    res.loc[res.anx=='somewhat anxious', 'anx'] = 2
    res.loc[res.anx=='very anxious', 'anx'] = 3
    ...
    Mnames = ['p_harm', 'anx', 'emo']
    print('p_harm', sorted(set(res.p_harm)))
    print('anx', sorted(set(res.anx)))
    ...
    res.loc[res.anx<2, 'anx'] = 0
    res.loc[res.anx>=2, 'anx'] = 1
    M = res[Mnames].values.astype(float)
    print('p_harm', Counter(res.p_harm))
    print('anx', Counter(res.anx))
    ...
    cv_path = 'patients_cv_split_real_data1.pickle'
        ...
        for cvi in range(len(tr_sids)):
            ...
            for pi, pm in enumerate(prediction_methods):          
                ...
                model_a_l, model_a_l_perf = fit_prediction_model(pm+':bclf', Ltr, Atr,
                                        save_path='models_real_data1/model_a_l_cv%d_%s'%(cvi+1, pm) if bti==0 else None,
                                        random_state=random_state+pi+1000)
            
                # fit Y|A,L,M
                model_y_alm, model_y_alm_perf = fit_prediction_model(pm+':ltr', np.c_[Atr, Ltr, Mtr], Ytr,
                                    save_path='models_real_data1/model_y_alm_cv%d_%s'%(cvi+1, pm) if bti==0 else None,
                ...
                for mi, mediator_name in enumerate(Mnames):
                    if mi==0:
                        ...
                        model_m_al, model_m_al_perf = fit_prediction_model(pm+':bclf', np.c_[Atr, Ltr], Mtr[:, mi],
                                    save_path='models_real_data1/med_model_%s_cv%d_%s'%(mediator_name, cvi+1, pm) if bti==0 else None,
            ...
            with open('results_real_data1.pickle', 'wb') as ff:
    ...
    # save
    with pd.ExcelWriter('results_real_data1.xlsx') as writer:
        for df in dfs:
